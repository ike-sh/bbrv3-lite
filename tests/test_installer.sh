...
case "$url" in
    ...
    */tags\?per_page=100) printf '[{"name":"v7.0.5"}]\n' ;;
    */releases/download/*) exit 22 ;;
    */v7.0.5/net-tcp-tune.sh) cp "$FIXTURE_ROOT/net-tcp-tune.sh" "$output" ;;
    */v7.0.5/SHA256SUMS)
...
"$PREFIX/bbr" version | grep -Fq 'v7.0.5' || { echo "FAIL: installed version mismatch" >&2; exit 1; }
run_output=$(PATH="$FAKE_BIN:$PATH" FIXTURE_ROOT="$ROOT_DIR" bash "$ROOT_DIR/install-alias.sh" --prefix "$RUN_PREFIX" --run 2>&1)
grep -Fq 'bbrv3-lite v7.0.5' <<< "$run_output" || { echo "FAIL: --run did not execute installed command" >&2; exit 1; }
