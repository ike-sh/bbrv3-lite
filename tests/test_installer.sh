...
case "$url" in
    ...
    */tags\?per_page=100) printf '[{"name":"v8.0.0"}]\n' ;;
    */releases/download/*) exit 22 ;;
    */v8.0.0/net-tcp-tune.sh) cp "$FIXTURE_ROOT/net-tcp-tune.sh" "$output" ;;
    */v8.0.0/SHA256SUMS)
...
grep -Fq 'bbrv3-lite v8.0.0' <<< "$run_output" || { echo "FAIL: --run did not execute installed command" >&2; exit 1; }
