...
case "$url" in
    ...
    */tags\?per_page=100) printf '[{"name":"v8.0.3"}]\n' ;;
    */releases/download/*) exit 22 ;;
    */v8.0.3/net-tcp-tune.sh) cp "$FIXTURE_ROOT/net-tcp-tune.sh" "$output" ;;
    */v8.0.3/SHA256SUMS)
...
"$PREFIX/bbr" version | grep -F 'v8.0.3' >/dev/null || { echo "FAIL: installed version mismatch" >&2; exit 1; }
if [[ $(id -u) == 0 ]]; then
...
grep -Fq 'bbrv3-lite v8.0.3' <<< "$run_output" || { echo "FAIL: --run did not execute installed command" >&2; exit 1; }
